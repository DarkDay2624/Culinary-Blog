...
CLOUDINARY_URL = "cloudinary://819189165356324:lKep8omjKP1VveVmyBo_vRz_8YM@khnure"
require('dotenv').load();
...
router.get("/", function (req, res) {
    if (req.query.search) {
        ...
        j_food.find({"name": regex}.sort({date: 1}), function (err, allj_foods) {
            if (err) {
                console.log(err);
            } else {
        ...
        //Get all j_foods fro the DB
        j_food.find({}, null, {sort: {"date": -1}},function (err, allj_foods) {
            if (err) {
                console.log(err);
            } else {
                res.render("j_foods/index", {j_foods: allj_foods});
...
router.post("/", middleware.isLoggedIn, upload.single('image'), function (req, res) {
    ...
    cloudinary.uploader.upload(req.file.path, function (result) {
        ...
        req.body.j_food.image = result.url;
